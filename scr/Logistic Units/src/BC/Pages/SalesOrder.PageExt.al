pageextension 71628576 "TMAC Sales Order" extends "Sales Order"
{
    layout
    {
        addbefore(SalesDocCheckFactbox)
        {
    ...
    actions
    {
        ...
        addbefore("F&unctions")
        {
            group("TMAC Logistic Units")
            {
                ...
                action("TMAC Include Logistics Units")
                {
                    ...
                    Image = Import;
                    ApplicationArea = All;
                    ToolTip = 'This function allows you to add lines (all items) from the selected logistic unit to the sales order. In doing so, links between the lines of this document and the logistic unit will be established.';
                    AccessByPermission = tabledata "TMAC Unit" = I;

                    trigger OnAction()
                    begin
                        UnitManagement.IncludeUnitInOrder(SourceType, SourceSubtype, SourceID, Database::"Sales Shipment Line");
                        CurrPage.Update();
                        UpdateLogisticUnitsSubform();
                    end;
                }
                ...
                action("TMAC Exclude Logistics Units")
                {
                    ...
                    Image = Export;
                    ApplicationArea = All;
                    ToolTip = 'Exclude content on the selected logistic units from the sales order.';
                    AccessByPermission = tabledata "TMAC Unit" = I;
        ...
        addafter(Post)
        {
            action("TMAC Post Logistic Unit")
            {
                ...
                begin
                    UnitPost.Post(SourceType, SourceSubtype, SourceID, Database::"Sales Shipment Line", false);
                end;
            }
        }
    ...
    begin
        ...
        CurrPage.SalesLines.Page.GetSelectedLineLinks(SourceDocumentLink);
    end;

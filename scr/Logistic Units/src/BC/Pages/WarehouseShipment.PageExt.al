pageextension 71628577 "TMAC Warehouse Shipment" extends "Warehouse Shipment"
{
    layout
    {
    ...
    actions
    {
        addafter(Category_Report)
        {
            group("TMAC Logistic Units Promoted")
            {
                ...
                actionref("TMAC Logistic Units New Promoted"; "TMAC New Logistic Units")
                {
                }
                actionref("TMAC Logistic Units Builder Promoted"; "TMAC Logistic Units Builder")
                {
                }
                ...
                actionref("TMAC Print Packing Promoted"; "TMAC Print Packing List")
                {
                }
            }
        }

        addafter("P&ost Shipment_Promoted")
        {
        ...
        addbefore("F&unctions")
        {
            ...
            group("TMAC Logistic Units")
            {
                ...
                action("TMAC Logistic Units Builder")
                {
                    Caption = 'Logistic Units Builder';
                    Image = Item;
                    ApplicationArea = All;
                    ToolTip = 'Create a new logistic unit (box, pallet, container) and add the selected lines from this order.  Lines can be selected in the Lines section using the Ctrl or Shift keys. To automatically enter the selected quantity using the value from the “Qty. to Ship" field, activate the “Set Default Selected Quantity” setting in the Logistic Unit Setup. To ignore empty (or distributed) lines without a selected quantity, use the setting "Exclude Lines w/o Def. Qty.".';
                    AccessByPermission = tabledata "TMAC Unit" = I;
                    ...
                    trigger OnAction()
                    var
                        LogisticUnitBuild: Page "TMAC Logistic Unit Builder";
                    begin
                        GetSelectedLineLinks();
                        LogisticUnitBuild.SetDocumentLinks(SourceDocumentLink, "TMAC Direction"::Outbound);
                        LogisticUnitBuild.Run();
                    end;
                }

                action("TMAC Add To Logistic Unit")
                {
                    Caption = 'Add To Logistic Unit';
                    Image = Item;
                    ApplicationArea = All;
                ...
                action("TMAC Print Packing List")
                {
                    Caption = 'Packing List';
                    Image = Print;
                    ApplicationArea = All;
                    ToolTip = 'This function will generate and print packing list by source document';
                    AccessByPermission = tabledata "TMAC Unit" = I;
                    ...
                    begin
                        ...
                        Report.Run(Report::"TMAC Packing List", true, true, WarehouseShipmentHeader);
                    end;
                }
            }
        }
        ...
        addafter("P&ost Shipment")
        {
            action("TMAC Post Logistic Unit")
            {
                ...
                begin
                    UnitPost.Post(SourceType, SourceSubtype, SourceID, Database::"Posted Whse. Shipment Line", false);
                end;
            }
        }
    }
    ...
    begin
        ...
        CurrPage.WhseShptLines.Page.GetSelectedLineLinks(SourceDocumentLink);
    end;
